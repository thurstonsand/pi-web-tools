...
- Deferred, acknowledged: the worker's concurrency state (context lifecycle, slots, drain, escalation gate, interactive ownership) lives as module globals in one 600-line file with smoke-test-only coverage. The agreed follow-up is extracting a browser-session state owner as the seam that makes those invariants unit-testable — a prefactor for any further worker features, not part of this design's delivery.

## Addendum: Remaining Work

Deliberately left out of this design's delivery, in dependency order:

1. **Extract the worker's state owner.** A `BrowserSession` (or similar) owning the browser context lifecycle, page slots, drain/escalation gate, interactive ownership, UA normalization, and shutdown — with the browser launcher injected so the concurrency logic runs against a fake. The socket layer shrinks to protocol plumbing. Behavior-preserving; validated against the existing SMOKE.md cases. This is the prefactor gate: no new worker feature lands before it.
2. **Concurrency test suite**, enabled by 1. Deterministic coverage for: slot handoff under contention; escalation drains peers, blocks new fetches, and releases on both success and failure; two simultaneous challenged fetches cannot deadlock (second retries headless after the first's escalation); launch failure rejects every queued request; the stall watchdog fires on a silent worker and not on a slow one; restart cannot race the next fetch; interactive-open behavior during active work. Today these live only in ad-hoc smoke scripts.
