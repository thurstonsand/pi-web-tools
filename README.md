...
- `search_web` — web search with results shaped for agent consumption.
- `fetch_web` — resolve a URL to a document, picking the best backend for the source.
...
- **Parallel API key** enables `search_web` and the Parallel fetch backend. Without it, `search_web` is not registered and fetches fall through to the local browser.
...
`fetch_web` routes each URL through a chain of fetchers that can handle certain kinds of urls. The first to produce a document wins. The chain is `github → parallel → local`.
